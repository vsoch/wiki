function cov_check()
% COVERAGE CHECKING for SPM ANALYSIS (cov_check.m)
% Vanessa Sochat
%
...
%
% You will also need to edit the 'Record Subjects to file...' section 
% based on the length of your subject IDs.  For example.
% ELIMINATED(i).Subject_ID(1,36:49) prints the subject ID that starts at
% character 36 and ends at character 49.
%__________________________________________________________________________
%
% USAGE: cov_check()
...
% Masks under EXP\Analysis\SPM\ROI
% 1st level analyzed folders w/ masks under EXP\Analysis\SPM\Analyzed\Task
% Output goes to EXP\Analysis\SPM\Second_Level\BOLD\(Task)\Coverage_check\(date)
%__________________________________________________________________________
%
%
...
%-Global Variables
%-----------------------------------------------------------------------
fprintf('COV_CHECK.M Alpha\n Vanessa Sochat\n July, 2010\n');

% Here the user specifies the experiment directory
homedir = spm_select(1,'dir','Select top of experiment directory','','N:');

% Here the user specifies the Task
if strcmp(homedir,'N:\AHABII.01\')
    task=spm_input('Task',1,'Faces|Cards|EmoReg');
else
    task=spm_input('Task',1,'Faces|Cards');
end
    
% If the user selects 'Faces' we need to choose block or affect design:
if strcmp(task,'Faces')
   if strcmp(homedir,'N:\AHABII.01\')
      task=spm_input('Faces Version',2,'Faces|OldFaces'); 
      design=spm_input('Design',3,'affect|block');
      task=horzcat(task,'\',design);
   else
    design=spm_input('Design',1,'affect|block');
    task=horzcat(task,'\',design);
   end
end

% Here the user specifies the number of subjects:
subj_count = spm_input('Number of subjects',2,'','',1);

% Here the user specifies the percentage coverage desired for each subject:
coverage = spm_input('Percent coverage desired',3,'',.95,1);

% Here the user specifies the subjects to be included in the coverage check
subj_dir = horzcat(homedir,'Analysis\SPM\Analyzed\');
subjects = spm_select(subj_count,'dir','Select subjects to check coverage for','',subj_dir);

% Each subject's mask.img should be under Analysis/SPM/Analyzed/(Task)
% This should be an array of subject IDs

% Here the user specifies the ROI to use
masks_dir = horzcat(homedir,'Analysis\SPM\ROI\');
ROI = spm_select(1,'.img','Select ROI to use in analysis','',masks_dir);
...
output_dir=horzcat(homedir,'Analysis\SPM\Second_level\BOLD\',task,'\');
cd(output_dir);
if (exist('Coverage_Check','dir')==0)
    fprintf('Coverage_Check directory does not exist.  Creating it!\n');
    mkdir('Coverage_Check');
end

% Create an output directory based on date and time.
cd Coverage_Check;
mkdir(horzcat(date,'_',datestr(clock, 'HH-MM')));
output_dir=horzcat(homedir,'Analysis\SPM\Second_level\BOLD\',task,'\Coverage_Check\',date,'_',datestr(clock, 'HH-MM'));
...
for su=1:subj_count
    ...
    s = deblank(s);
    subj_length=length(s);
    if strcmp(s(subj_length),'\')==0
       s = [ s '\' ];
    end
    ...
    if exist('mask.img','file')
        
        % Copy the .hdr
        subj_mask=horzcat('mask_',num2str(su),'.hdr');  
        subj_mask_dir=horzcat(output_dir,'\masks\',subj_mask);
        copyfile('mask.hdr',subj_mask_dir);
        
        % Copy the .img
        subj_mask=horzcat('mask_',num2str(su),'.img');
        subj_mask_dir=horzcat(output_dir,'\masks\',subj_mask);
        copyfile('mask.img',subj_mask_dir);
...
% based on coverage percentage.    
        ...
        if (smask_count >= vox_min)
            ...
            fprintf('\n');
            per_cov= smask_count/ROI_count,3;
...
if exist('MISSING','var')~= 0
    % Returns array of all locations of '\' in subject path string
    miss_markers=regexp(MISSING(1).Subject_ID, '\');
    % Returns the start of the subject ID
    miss_start=miss_markers(size(miss_markers,2)-1)+1;
    % Returns the end of the subject ID
    miss_end=miss_markers(size(miss_markers,2))-1;
    ...
    for i=1:numel(MISSING)
        fprintf(fid, '%s\t%d\t\n', MISSING(i).Subject_ID(1,miss_start:length(MISSING(i).Subject_ID)-1), MISSING(i).Mask_Number);
    end
    fclose(fid);
end
...
if exist('INCLUDED','var')~= 0
    % Returns array of all locations of '\' in subject path string
    inc_markers=regexp(INCLUDED(1).Subject_ID, '\');
    % Returns the start of the subject ID
    inc_start=inc_markers(size(inc_markers,2)-1)+1;
    % Returns the end of the subject ID
    inc_end=inc_markers(size(inc_markers,2))-1;
    ...
    fprintf(fid, 'Coverage Minimum for Inclusion: ');
    fprintf(fid, '%g\n\n', vox_min);
    fprintf(fid, 'Subject_ID        Mask_Number Voxels  Percent_Coverage\n');
    for i=1:numel(INCLUDED)
        fprintf(fid, '%s\t%d\t\t%d\t%.3f\n', INCLUDED(i).Subject_ID(1,inc_start:length(INCLUDED(i).Subject_ID)-1), INCLUDED(i).Mask_Number, INCLUDED(i).Voxels, INCLUDED(i).Percent_Coverage);
    end
    fclose(fid);
end
...
if exist('ELIMINATED','var')~= 0
    % Returns array of all locations of '\' in subject path string
    elim_markers=regexp(ELIMINATED(1).Subject_ID, '\');
    % Returns the start of the subject ID
    elim_start=elim_markers(size(elim_markers,2)-1)+1;
    % Returns the end of the subject ID
    elim_end=elim_markers(size(elim_markers,2))-1;
    ...
    for i=1:numel(ELIMINATED)
        fprintf(fid, '%s\t%d\t\t%d\t%.3f\n', ELIMINATED(i).Subject_ID(1,elim_start:length(ELIMINATED(i).Subject_ID)-1), ELIMINATED(i).Mask_Number, ELIMINATED(i).Voxels, ELIMINATED(i).Percent_Coverage);
    end
    fclose(fid);
end
...
if exist('INCLUDED','var')~= 0
    ...
    fprintf(fid, 'Subject_ID        Mask_Number Voxels\n');
    for i=1:numel(INCLUDED)
        fprintf(fid, '%s\t%d\t\t%d\n', INCLUDED(i).Subject_ID(1,inc_start:inc_end), INCLUDED(i).Mask_Number, INCLUDED(i).Voxels);
    end
    fclose(fid);
end
fprintf('%s\n','Done running cov_check.m');
end