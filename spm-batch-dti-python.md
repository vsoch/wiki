...
#------SPM_BATCH_DTI.py---------------------------
#
# This script is used for preprocessing single subject DTI data
...
template_f = file("spm_batch_DTI_TEMPLATE.sh")      #job template location (on head node)
...
subnums = ["22222222_11111"]
runs = [1]               #[ run01 ] range cuts the last number off any single runs should still be in [ ] or can be runs=range(1,2)

tworuns="yes"                #"Yes" indicates that the subject has two DTI folders to process, "no" has one
dtifolder1 = "series009"     #folder under Data/Anat that includes the first set of raw dw images.
dtifolder2 = "series010"     #folder under Data/Anat that incudes the second set of raw dw images
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_DTIFOLDER1_SUB", str(dtifolder1) )
		tmp_job_file = tmp_job_file.replace( "SUB_DTIFOLDER2_SUB", str(dtifolder2) )	
		tmp_job_file = tmp_job_file.replace( "SUB_TWODATASETS_SUB", str(tworuns) )
		 
		#make fname and write job file to cwd
		tmp_job_fname = "_".join( ["SPM_DTI", subnum, runstr ] )
