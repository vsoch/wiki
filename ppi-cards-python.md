...
#------PPI_CARDS.py---------------------------
#
# This script works with it's partner python, PPI_CARDS.sh, and a MATLAB script 
# PPI_CARDS.m (in Scripts folder on Munin) to run PPI analysis.  It
# uses the PPI "button" in SPM to first extract a timeseries from
# based on a user specified roi.  This is a 7 regressor model
# This script is only for CARDS.  If you want to run faces, please use
# PPI_FACES or PPI_ALLFACES.  To minimize user error in entering contrast 
# matrices, Vanessa decided to hard code the matrices into the .m script, 
# The script runs PPI and then moves into a single subject analysis, 
# and uses the  extracted values (under PPI.P, PPI.ppi, and PPI.Y) as 
# regressors, for a total of 7 conditions/regressors from PPI.  Additionally, 
...
template_f = file("PPI_CARDS.sh")            #job template location (on head node)
...
# the subnums below are for DNS 200 freeze
...
task = "cards"                 # MUST be either "faces" or "cards" or "rest" SCRIPT ONLY WORKING WITH FACES CURRENTLY!
outputfolder = "bilateral_VS"
#outputfolder="VS_sphere_200"  # This is the name that you would like for the output folder under "Analyzed/Subject/PPI"
...
conname = "PosFbk_gr_NegFbk"      # Name of contrast of interest (PosFbk_gr_NegFbk)
...
fcontrast = "5"                # Index of F-contrast number used to adjust data GUI default is 0, Cards EOI is 5
...
# VOI Variables
voiname = "bilateral_VS"
#voiname = "VS_sphere_200"     # Name that we want our resulting VOI to have (VOI_ is auto. prefixed)
                               # You should append the number of subjects used to create the group mask "rmask_cluster_141"
...
subincluded ="171s"            # The name of the subject-number-folder under ROI/PPI/Task/ with masks in it ("141s")
maskname = "bilateral_VS.img"
#maskname ="VS_sphere.nii"     # Tbe full name (including extension) of a mask to be used to define VOI
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_fname = "_".join( ["PPI_CARDS", subnum, runstr ] )
