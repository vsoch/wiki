Please note that this is also an old python submission script that was not updated for the new cluster.
...
# it works with PREPROCESS_FEAT_TEMPLATE.sh to run multiple subjects
#
...
template_f = file("PREPROCESS_FEAT_TEMPLATE.sh")  #job template location (on head node)
...
subnums = ["11111111_22222"] #should be entered in quotes, separated by commas, to be used as strings
runs = [1] #[ run01 ] range cuts the last number off any single runs should still be in [ ] or can be runs=range(1,2)
anatfolder = "series002" #this is the folder under Data/Anat that includes the BET folder with the anatomical
anatfile = "anat_brain" # this is the name to give the brain extraction (convention is "anat_brain")
funcfolder = "run004_03" #this is the name of the folder to convert
volume = "195" #This is the image number at the end of the functional run that must be deleted MINUS one. MUST be in the format "012"
# If you want to delete "172" for example, specify "171" as the count starts at 0
#if there are no images to be deleted, enter a value of "0" 
#(the scanner must have an even number of images, and some tasks have an odd number of timepoints)
betvalue = ".225" #This is the threshold that you want to use for the brain extraction
design = "Block"  #This specifies if the Design is Block, Event, or another type
task = "TASK" #This is the name of the task under Analysis
order ="1"  #This is the order that the above task was run with.  If there is just one order, put "1"

# processing choices
anatpre ="no" #Put "yes" if you want to preprocess anatomical directory, "no" if not
funcpre = "yes" #Put "yes" if you want to preprocess the functional data and create nifti headers. "no" if not
featrun = "no" #Put "yes" if you want to run first level FEAT
####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!###############################################
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_ANATFOLDER_SUB", str(anatfolder) )
 		tmp_job_file = tmp_job_file.replace( "SUB_ANATFILE_SUB", str(anatfile) )
 		tmp_job_file = tmp_job_file.replace( "SUB_FUNCFOLDER_SUB", str(funcfolder) )
 		tmp_job_file = tmp_job_file.replace( "SUB_VOLUME_SUB", str(volume) )
 		tmp_job_file = tmp_job_file.replace( "SUB_BETVALUE_SUB", str(betvalue) )
 		tmp_job_file = tmp_job_file.replace( "SUB_RUNNAME_SUB", str(run) )
		tmp_job_file = tmp_job_file.replace( "SUB_DESIGN_SUB", str(design) )
		tmp_job_file = tmp_job_file.replace( "SUB_TASK_SUB", str(task) )
		tmp_job_file = tmp_job_file.replace( "SUB_ORDER_SUB", str(order) )
		tmp_job_file = tmp_job_file.replace( "SUB_ANATPRE_SUB", str(anatpre) )
		tmp_job_file = tmp_job_file.replace( "SUB_FUNCPRE_SUB", str(funcpre) )
		tmp_job_file = tmp_job_file.replace( "SUB_FEATRUN_SUB", str(featrun) )
		 
		#make fname and write job file to cwd
		tmp_job_fname = "_".join( ["PREPROCESS_FEAT", subnum, runstr ] )
