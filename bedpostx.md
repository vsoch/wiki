...
BedpostX is the second step in the DTI processing pipeline that sets the stage for probabilistic tractography.
  * You can read about how it works [here](http://www.fmrib.ox.ac.uk/fsl/fdt/fdt_bedpostx.html)
  * To run this script, we use `BedpostX.py`, which uses `BedpostX_TEMPLATE.sh` as... a template!
  * To do a single run, use `BedpostX.sh`
  * All three scripts are included on this page!
