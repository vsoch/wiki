...
# the GroupFEAT/name.gfeat/Cluster_masks directory with the format
...
# [abc1@head ~]$ qsub -v EXPERIMENT=Dummy.01 fq.sh FACES   AllOnesNoEyes      AllOnes        LAmyVentral     run01   1111111111
#                                                 Design   FEAT Design        Group_FEAT_design  GFEAT runname FEATrunname SUBJECT 
...
# THINGS THAT YOU MUST CHANGE! (at least without python script)
COPES="1" # Faces > SHAPES, etc.  #These are the copes specified in the GroupFEAT
MASKS="1" # These are the numbers of the masks that you have created and want to use in featquery
...
for COPE in $COPES; do
	...
	ROIPATH=$GFEAT/cope$COPE.feat
		...
		for MASK in $MASKS; do
			...
			OUTPATH=$EXPERIMENT/Analysis/ROI/$GFEAT_DESIGN/$GFEAT_RUN/cope$COPE/Cluster_$MASK/$SUBJECT
