...
subnums = ["YYYYMMDD_11111"] #should be entered in quotes, separated by commas, to be used as strings
runs = [1]                   #[ run01 ] range cuts the last number off any single runs should still be in [ ] or can be runs=range(1,2)
anatfolder = "series002"     #folder under Data/Anat that includes the anatomical dicom images
tone = "series005"           #t1 folder (series005) if it doesn't exist, leave blank
facesfolder = "run00X_0X"    #folder with the faces functional data (in V00*.img/.hdr format)
cardsfolder = "run00X_0X"    #folder with the cards functional data (in V00*.img/.hdr format)
restfolder = "run00X_0C"     #folder with the rest functional data (in V00*.img/.hdr format)
...
restrun = "yes"              #"yes" runs preprocessing and single subject analysis for faces, "no" skips
justfunc = "no"             #"yes" skips anatomical processing.  To be used if you have manually set origin of anatomicals
