...
# --------------PPI ALL FACES TEMPLATE ----------------
#
# This script works with it's partner python, PPI_ALLFACES.py, and a MATLAB script 
...
# Here we check that the user is running faces.  This script does not support other tasks! 
# Since the all faces > shapes matrix is the same for everyone, we don't need to know the order number.
case $ATASK in
...
# ------------------------------------------------------------------------------
#  Step 2: Creation and run of PPI_ALLFACES subject script
# ------------------------------------------------------------------------------
...
sed -e 's@SUB_MOUNT_SUB@'$EXPERIMENT'@g' \
 ...
 -e 's@SUB_SUBJECT_SUB@'$SUBJ'@g' <$i> $OUTDIR/PPI_ALLFACES.m
 done
...
/usr/local/bin/matlab -display :$RANDINT < PPI_ALLFACES.m;

echo "Done running PPI_ALLFACES.m in matlab"
