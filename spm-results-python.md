...
#------SPM_RESULTS.py---------------------------
#
# This script is used for creating results reports
# with spm_results.sh 
...
template_f = file("spm_results.sh")             #job template location (on head node)
...
tonerun = "yes"              #"yes" runs preprocessing and single subject analysis for faces, "no" skips
####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!###############################################
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_TONERUN_SUB", str(tonerun) )
		...
		tmp_job_fname = "_".join( ["RES_RPT", subnum, runstr ] )
